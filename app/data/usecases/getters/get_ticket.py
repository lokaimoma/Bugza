...
async def get_tickets_by_project_id(session: AsyncSession, project_id: int) -> List[Ticket]:
    query = select(Ticket).where(Ticket.project_id == project_id).options(selectinload(Ticket.project))
    result = await session.execute(query)
    return result.scalars().all()


async def get_tickets(session: AsyncSession, page_number: int) -> List[Ticket]:
    query = select(Ticket).limit(ROWS_PER_PAGE).offset(ROWS_PER_PAGE * page_number).options(
...
async def get_ticket_by_id(session: AsyncSession, ticket_id: int) -> Optional[Ticket]:
    query = select(Ticket).where(Ticket.id == ticket_id).options(selectinload(Ticket.project))
    result = await session.execute(query)
